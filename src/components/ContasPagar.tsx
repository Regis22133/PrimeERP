...
const ContasPagar: React.FC = () => {
  ...
  const overdueMetrics = useMemo(() => {
    ...
    const overdueTransactions = expenseTransactions.filter(t => t.dueDate < now);
    
    const totalPayables = expenseTransactions.reduce(
    ...
    return {
      ...
      rate: totalPayables.isZero() ? 0 : overdueAmount.dividedBy(totalPayables).times(100).toNumber(),
      aging,
      transactions: overdueTransactions
    };
  }, [transactions]);

  const expenseCategories = categoryTypes.filter(cat => cat.type === 'expense');
  const activeCostCenters = costCenters.filter(cc => cc.active);
  const suppliers = contacts.filter(c => c.type === 'supplier');

  const searchColumns: SearchColumn[] = [
    { id: 'all', label: 'Todas as Colunas', field: 'description' },
    { id: 'supplier', label: 'Fornecedor', field: 'supplier' },
  ...
  const filteredTransactions = transactions.filter(transaction => {
    ...
    return matchesType && matchesDate && isPending && matchesSearch;
  });
  ...
  const handleExport = () => {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `contas_a_pagar_${new Date().toISOString().split('T')[0]}.xlsx`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const validCategories = new Set(expenseCategories.map(c => c.name));
      const validCostCenters = new Set(activeCostCenters.map(c => c.name));
      const validSuppliers = new Set(suppliers.map(s => s.name));
      ...
      transactions.forEach((t, index) => {
        ...
        if (t.supplier && !validSuppliers.has(t.supplier)) {
          errors.push(`Linha ${row}: Fornecedor "${t.supplier}" não encontrado`);
        }
      });
          ...
          <p className="text-gray-500 mt-1">
            {overdueMetrics.count > 0 ? (
              <span className="text-red-600">
                {overdueMetrics.count} pagamentos em atraso, totalizando{' '}
                {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' })
                  .format(overdueMetrics.amount)}
                {' '}({overdueMetrics.rate.toFixed(1)}% do total a pagar)
              </span>
            ) : (
        ...
        <div className="mb-6 grid grid-cols-4 gap-4">
          <div className="bg-red-50 p-4 rounded-lg border border-red-100">
            ...
            <p className="text-sm text-red-600 mt-1">
              {overdueMetrics.aging['1-30'].count} pagamentos
            </p>
          </div>
          ...
          <div className="bg-red-50 p-4 rounded-lg border border-red-100">
            ...
            <p className="text-sm text-red-600 mt-1">
              {overdueMetrics.aging['31-60'].count} pagamentos
            </p>
          </div>
          ...
          <div className="bg-red-50 p-4 rounded-lg border border-red-100">
            ...
            <p className="text-sm text-red-600 mt-1">
              {overdueMetrics.aging['61-90'].count} pagamentos
            </p>
          </div>
          ...
          <div className="bg-red-50 p-4 rounded-lg border border-red-100">
            ...
            <p className="text-sm text-red-600 mt-1">
              {overdueMetrics.aging['>90'].count} pagamentos
            </p>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg w-full max-w-2xl">
            <h2 className="text-xl font-bold mb-4">
              {editingTransaction ? 'Editar Pagamento' : 'Novo Pagamento'}
            </h2>
